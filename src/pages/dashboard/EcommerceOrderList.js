...
import {
  ...
  ProductListHead,
  ProductListToolbar,
} from '../../sections/@dashboard/e-commerce/order-list';
...
const TABLE_HEAD = [
  ...
  { id: 'Order Item', label: 'Items', alignRight: false },
  { id: 'isDelivered', label: 'Delivered', alignRight: false },
  { id: 'isPaid', label: 'Paid', alignRight: false },

  { id: 'payment', label: 'Payment', alignRight: false },
  { id: 'total', label: 'Price', alignRight: true },
  { id: '' },
];
              ...
              <Table>
                ...
                <TableBody>
                  {filteredProducts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    ...
                    const { _id, payment,user, orderItems,orderid, total,isDelivered,isPaid, createdAt } = row;
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell style={{ minWidth: 80 }}>{(orderItems.length)}</TableCell>
                        <TableCell style={{ minWidth: 90 }}>
                          <Label
                            variant={theme.palette.mode === 'light' ? 'ghost' : 'filled'}
                            color={
                              (isDelivered === true && 'success') ||
                              (isDelivered === false && 'error') 
                            }
                          >
                            {sentenceCase(isDelivered?'true':'false')}
                          </Label>
                        </TableCell>
                          <TableCell style={{ minWidth: 90 }}>
                          <Label
                          ...
                          >
                            {sentenceCase(isPaid?'true':'false')}
                          </Label>
                        </TableCell>
                        <TableCell  sx={{ display: 'flex', alignItems: 'center' }}>{payment.paymentMethod}</TableCell>
                        <TableCell>{fCurrency(total)}</TableCell>
                        <TableCell align="right">
                            <ProductMoreMenu productName={orderid} codeName={_id} onDelete={() => handleDeleteProduct(_id)} /> 
                        </TableCell>
                      </TableRow>
                    );
                  })}

...
console.log(product);
  ...
  const isMaxQuantity = cart.filter((item) => item._id === _id).map((item) => item.quantity)[0] >= available;
  ...
  const linkTo = `${PATH_DASHBOARD.eCommerce.root}/product/${paramCase(name)}/${code}/edit`;
  return (
    <RootStyle {...other}>
